...
// 讀取 live_site 及 trial_site 的設定檔
const site_config = require("./data/siteConfig.json");
...
// 讀取 firebse service account key file
const firebase_key_file = require(`./data/${project_id}/firebase_service_account_key.json`);

// 是否為 live site
let b_live_site = false;
if (project_id == site_config.live_site) b_live_site = true;

// 建立 firebase DB 連線
let firebaseConfig = {
  credential: admin.credential.cert(firebase_key_file)
};

firebaseConfig.databaseURL = `https://shop-caller-${project_id}.asia-southeast1.firebasedatabase.app/`;
let callerApp = admin.initializeApp(firebaseConfig, 'callerApp');
let callerDB = getDatabase(callerApp);

firebaseConfig.databaseURL = `https://callnum-log-${project_id}.asia-southeast1.firebasedatabase.app/`;
let callnumApp = admin.initializeApp(firebaseConfig, 'callnumApp');
let callnumDB = getDatabase(callnumApp);

// 若為 live site，需取得 callerDB_trial，以便將 Caller 資料寫入 trial site 的 firebase
let callerDB_trial;
if (b_live_site) {
  let project_id_trial = site_config.trial_site;
  const firebase_key_file_trial = require(`./data/${project_id_trial}/firebase_service_account_key.json`);
  let firebaseConfig_trial = {
    credential: admin.credential.cert(firebase_key_file_trial)
  };
  firebaseConfig_trial.databaseURL = `https://shop-caller-${project_id_trial}.asia-southeast1.firebasedatabase.app/`;
  let callerApp_trial = admin.initializeApp(firebaseConfig_trial, 'callerApp_trial');
  callerDB_trial = getDatabase(callerApp_trial);
}
...
function NewSocket(i) {
  try {
    ...
    */
    ...
    ws[i].onerror = (e) => {
      console.log('[WebSocket '+i+'] error occurred');
      ws[i] = null;
    };

    ws[i].onmessage = ws_onmessage;
    
    console.log("Websoket "+i+" created");
  }
  catch (err) {
    console.log('error: failed to create Websoket '+i);
    ...
    for (i = 0; i < d.length; i++) {
      ...
      if (d[i].call_nums != null) {
        ...
        */
        ...
        // 若為 live site，需額外寫入 callerDB_trial
        if (b_live_site) {
...
async function query_mainpi() {
  ...
  // 若不是 live site，只查詢少數店家
  if (!b_live_site) msg_arr = [query_msgs[0]];
...
async function main() {
  ...
  // 判斷是否為多廠家
  let b_multi_factory = false;
  const [files] = await storage.bucket(bucketName).getFiles({prefix: 'shop_data/shop_data_'});
  //console.log(files);
  if (files.length > 0) b_multi_factory = true;
  
  // shop_data.json 檔案路徑
  let shop_file_path = path.join(process.cwd(), '../common_data/shop_data.json');
  if (b_multi_factory) shop_file_path = path.join(process.cwd(), '../common_data/shop_data');
  ...
  // 上傳 callnum_log 到 Cloud Storage
  async function upload_callnum_file(file_name) {
    const options = {
      destination: "callnum_log/"+file_name
    };
    let file_path = path.join(process.cwd(), '../common_data/callnum_log/'+file_name);
    
    try {
      await storage.bucket(bucketName).upload(file_path, options);
      console.log(`gs://${bucketName}/callnum_log/${file_name} uploaded`);
    }
    catch (err) {
      console.log(`error: failed to upload gs://${bucketName}/callnum_log/${file_name}`);
      console.log(err.message);
      log_file.write(`error: failed to upload gs://${bucketName}/callnum_log/${file_name}\n`);
      log_file.write(err.message+"\n");
    }
  ...
  // 移除 Cloud Storage 過期的 callnum_log 
  async function delete_expired_callnum_files(exp_ymd) {
    const listOptions = {
      prefix: "callnum_log/"
    };
  ...
  // 移除 Cloud Storage 過期的 shop_history
  async function delete_expired_shop_history(exp_ymd) {
    const listOptions = {
      prefix: "shop_history/"
    };
  ...
  async function read_new_shop_data() {
    await sleep(5000);
    let new_shop_file = await fs.readFileSync(shop_file_path, 'utf8');
  ...
  // 儲存 callnum_log 至檔案, 並清除過期資料 (包含 shop_history)
  async function save_callnum_log() {
    // 每天 5:00 ~ 6:00 執行
    let date_obj = new Date();
    let hour = ("00"+date_obj.getHours()).substr(-2);
    ...
    if (hour == "05") {
      ...
      /*
      // 測試資料
      callnumData['20240916']=
      {
        "204-0": {
          "16:59:38": {
            "call_num": 50,
            "time_diff": 12776606,
            "timestamp": "1727686778803"
          },
          "17:01:59": {
            "call_num": 51,
            "time_diff": 140820,
            "timestamp": "1727686919623"
          }
        }
      };
      callnumData['20240923']=
      {
        "207-0": {
          "15:40:36": {
            "call_num": 0,
            "time_diff": 14030585,
            "timestamp": "1727682036479"
          },
          "16:02:46": {
            "call_num": 1,
            "time_diff": 1330061,
            "timestamp": "1727683366540"
          }
        }
      };
      callnumData['20240924']=
      {
        "207-0": {
          "15:40:36": {
            "call_num": 0,
      ...
      */
      ...
      let save_data = {};
      // 假如 callnumData (記憶體) 有前一天的完整資料，則使用 callnumData
      if ((log_ymd in callnumData) && (log_ymd!=date_arr[0])) {
        save_data = callnumData[log_ymd];
      }
      else {  // 否則需從 firebase 的 callnumDB 取得 前一天 的資料
        let log_ymd_snapshot = await callnumDB.ref(`/${log_ymd}`).once('value');
      ...
      if (Object.keys(save_data).length > 0) {
        let path = `../common_data/callnum_log/${log_ymd}.json`;
        let data = JSON.stringify(save_data, null, 2);

        // 儲存 callnum_log 到 json 檔
        let save_OK = false;
        try {
          
          /*
          // 測試資料
          fs.writeFileSync('../common_data/callnum_log/20240916.json', data);
          fs.writeFileSync('../common_data/callnum_log/20240923.json', data);
          fs.writeFileSync('../common_data/callnum_log/20240924.json', data);
          */
          
          fs.writeFileSync(path, data);
          console.log(`callnum_log ${log_ymd}.json saved`);
          save_OK = true;
        }
        catch (err) {
          console.log(`error: failed to save callnum_log ${log_ymd}.json`);
          console.log(err.message);
          log_file.write(`error: failed to save callnum_log ${log_ymd}.json\n`);
          log_file.write(err.message+"\n");
        }

        // 上傳 callnum_log 到 Cloud Storage
        if (save_OK && (deploy_type == "cloud_run" || deploy_type == "docker_run")) {
          await upload_callnum_file(`${log_ymd}.json`);
        }          
      }

      // 計算 callnumData 過期日期 (保留一週)
      let exp_date_obj = new Date(Date.now()-1000*60*60*24*8);
      let exp_ymd = get_ymd(exp_date_obj);
      console.log("callnumData 過期日期："+exp_ymd);
      log_file.write("callnumData 過期日期："+exp_ymd+"\n");

      // 刪除 Object callnumData 過期資料
      ...
      // 計算 callnum_log 過期日期 (保留100天)
      exp_date_obj = new Date(Date.now()-1000*60*60*24*101);
      exp_ymd = get_ymd(exp_date_obj);
      console.log("callnum_log 過期日期："+exp_ymd);
      log_file.write("callnum_log 過期日期："+exp_ymd+"\n");
      
      // (Cloud Storage) 移除 callnum_log 過期 json 檔
      if (deploy_type == "cloud_run" || deploy_type == "docker_run") {
        await delete_expired_callnum_files(exp_ymd);
      }

      // (Local) 移除 callnum_log 過期 json 檔
      if (deploy_type == "cloud_run" || deploy_type == "docker_run") { // 若是 CloudRun, Local檔案只保留一週          
        exp_date_obj = new Date(Date.now()-1000*60*60*24*8);
        exp_ymd = get_ymd(exp_date_obj);
        console.log("Local json檔 過期日期："+exp_ymd);
        log_file.write("Local json檔 過期日期："+exp_ymd+"\n");
      }
      ...
      for (let i=0; i<cl_filenames.length; i++) {
        ...
        if (fname <= exp_ymd) {
          let path = `../common_data/callnum_log/${cl_filenames[i]}`;
          try {
            fs.unlinkSync(path);
            console.log(`callnum_log ${cl_filenames[i]} deleted`);
          }
          catch (err) {
            console.log(`error: failed to delete callnum_log ${cl_filenames[i]}`);
            console.log(err.message);
            log_file.write(`error: failed to delete callnum_log ${cl_filenames[i]}\n`);
            log_file.write(err.message+"\n");
          }
        }
      }
      ...
      // (Firebase) 移除 callnum_log 過期資料 
      callnumDB.ref(`/${exp_ymd}`).remove();
      console.log(`[firebase] remove callnum_log data: ${exp_ymd}`);
      log_file.write(`[firebase] remove callnum_log data: ${exp_ymd}\n`);
      ...
      // (Cloud Storage) 移除 shop_history 過期 json 檔
      if (deploy_type == "cloud_run" || deploy_type == "docker_run") {
        await delete_expired_shop_history(exp_ymd);
      }

      /*
      // 測試資料
      let test_data = JSON.stringify(shopData, null, 2);
      fs.writeFileSync('../common_data/shop_history/shop_data_20240615.json', test_data);
      fs.writeFileSync('../common_data/shop_history/shop_data_20240622.json', test_data);
      fs.writeFileSync('../common_data/shop_history/shop_data_20240623.json', test_data);
      */

      // (Local) 移除 shop_history 過期 json 檔
      ...
      for (let i=0; i<sh_filenames.length; i++) {
        ...
        if (fname.substr(10) <= exp_ymd) {
          let path = `../common_data/shop_history/${sh_filenames[i]}`;
          try {
            fs.unlinkSync(path);
            console.log(`shop_history ${sh_filenames[i]} deleted`);
          }
          catch (err) {
            console.log(`error: failed to delete shop_history ${sh_filenames[i]}`);
            console.log(err.message);
            log_file.write(`error: failed to delete shop_history ${sh_filenames[i]}\n`);
            log_file.write(err.message+"\n");
          }
        }
      }
  ...
  if (deploy_type == "cloud_run") {
    ...
    // 儲存 callnum_log
    save_callnum_log();
    setInterval(save_callnum_log, 1000*60*60);  

    // 查詢叫號機資料
    query_mainpi();
    setInterval(query_mainpi, 1000*60);
  }
}
