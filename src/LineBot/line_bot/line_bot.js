...
async function main() {
  ...
  */
  ...
  function get_booking_data(shop_id, user_id) {
    ...
    // 找不到 shop_id 資料
    if (!(shop_id in shop_data)) return {"booking_url": "", "booking_phone": ""};
    ...
    if (booking_url != "") {
      ...
      else if (index == booking_url.length-1) booking_url += `callme_id=${user_id_base64}`;
        ...
        switch (msgType) {
          ...
          case 'action':
            ...
            if (action == '叫號查詢') {
              ...
              // 讀取 預約暫存資料
              let booking_obj = {};	
              let snapshot = await bookingTempDB.ref(`${date_key}/${user_name}/${booking_time}`).once('value');
              if ((snapshot.exists()) && (snapshot!=null)) {
                booking_obj = snapshot.val();
                if (Object.keys(booking_obj).length == 0) {
                  console.log('booking data not exist!');
              ...
              */
              ...
              // console.log("booking_obj: "+JSON.stringify(booking_obj));
              ...
              if (Object.keys(booking_obj).length > 0) {
                ...
                let booking_num = booking_obj.booking_num;
                ...
                if (booking_time != "") msg += `\n時間：${booking_time}`;
                if (booking_num!="" && booking_num!="-1") msg += `\n號碼：${booking_num}`;
                msg += "\n\n將於預約日前一天☀️𝟵:𝟬𝟬提醒您";
                msg += "\n(逾上述時間後預約，將不再提醒)";
                //if (msg != "") {
                ...
                */
                ...
                // 準備通知訊息
                let booking_url = get_booking_data(shop_id, user_id).booking_url;
                replyMsgs.push(reply_msg.booking_info(target_name, user_name, booking_date, week_day, booking_time, booking_num, booking_url));
  ...
  */
  ...
  app.get('/booking_remind', async (req, res) => {
    let line_id = "@callmeback";
    if (project_id == "callme-op-419108") line_id = "@046sovlo";
    let booking_data = req.query.booking_data;
    console.log(`提醒我：${booking_data}`);
    res.render('booking_remind', {line_id:line_id, booking_data:booking_data});
  });
  
  app.get('/booking_remind_pc', async (req, res) => {
    let line_id = "@callmeback";
    if (project_id == "callme-op-419108") line_id = "@046sovlo";
    let booking_data = req.query.booking_data;
    let qrcode_url = `https://line.me/R/oaMessage/${line_id}/?預約提醒:${booking_data} 🔔 請發送此訊息，完成提醒設定`;
    const opts = {margin:3, maskPattern:0, errorCorrectionLevel:'L', scale:5, color:{dark:"#06C755",light:"#FFFFFF"}};
    let image_url = await QRCode.toDataURL(qrcode_url, opts);
    console.log('PC端：顯示QR-code');
    res.render('booking_remind_pc', {line_id:line_id, booking_data:booking_data, image_url:image_url});
  });

  /* 
  // (未採用) 圖文選單 "搜尋店家" 
  app.get('/shops', (req, res) => {
    res.sendFile(path.join(process.cwd(), '/data/shops.html'));
  });
  */
