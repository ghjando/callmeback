...
import * as reply_msg from './reply_msg_new.js';
...
function search_shop_by_pinyin(shop_data, query_str) {
  ...
  for (let i=0; i<pinyin_arr.length; i++) {
    pinyin_str += pinyin_arr[i] + " "; 
  }
  pinyin_str = pinyin_str.substr(0, pinyin_str.length-1);
  console.log(pinyin_str);

  // 搜尋店家
  let result_obj = {};
  for (let shop_id in shop_data) {
    if (shop_data[shop_id] && shop_data[shop_id].pinyin && shop_data[shop_id].pinyin.includes(pinyin_str)) {
      result_obj[shop_id] = shop_data[shop_id].name;
    }
  }
...
function search_shop_by_pinyin(shop_data, query_str, max_num) {
  ...
  // 搜尋店家
  for (let shop_id in shop_data) {
    if (shop_data[shop_id] && shop_data[shop_id].pinyin) {
      let index = shop_data[shop_id].pinyin.indexOf(pinyin_str);
      if (index != -1) {
        result_obj[shop_id] = shop_data[shop_id].name;
        index_arr.push({'index':index, 'shop_id':shop_id});
      }
