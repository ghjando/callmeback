    ...
    */
    ...
    async function save_usage_log() {
      ...
      if (hour == "04") {
        ...
        // 計算 usage_log 過期日期 (63天)
        let exp_date_obj = new Date(Date.now()-1000*60*60*24*63);
          ...
          if ('callers' in shop_data[shop_id]) {
            ...
            if (caller_id in callers) {
              ...
              if (notify_num !== "") {
                ...
                replyMsgs.push(reply_msg.caller_info(shop_id, caller_id, shop_name, address_fix, caller_name, curr_num, prev_num, change_time, update_time, last_update));
                replyMsgs.push({
                  type: 'text',
                  //text: `請輸入你的號碼...`
                  //text: `到幾號要叫你呢？\n(為避免過號，請輸入您希望提前通知的號碼)`
