...
// 製作 店家使用數季報
async function create_usage_season_report() {
      ...
      if (fs.existsSync(u_log_path)) {
        ...
        for (let shop_id in u_log_data) {
          for (let user_id in u_log_data[shop_id]) {
            if (shop_id in u_count_obj[j]) u_count_obj[j][shop_id] += Object.keys(u_log_data[shop_id][user_id]).length;
            else u_count_obj[j][shop_id] = Object.keys(u_log_data[shop_id][user_id]).length;
          }
        }    
      }
    }
  ...
  for (let shop_id in shop_data) {
    ...
    usage_season_report.push({
      'id': shop_id,
      'name': shop_name,
      'week9':(shop_id in u_count_obj['9'])? u_count_obj['9'][shop_id]:0,
      'week8':(shop_id in u_count_obj['8'])? u_count_obj['8'][shop_id]:0,
      'week7':(shop_id in u_count_obj['7'])? u_count_obj['7'][shop_id]:0,
      'week6':(shop_id in u_count_obj['6'])? u_count_obj['6'][shop_id]:0,
      'week5':(shop_id in u_count_obj['5'])? u_count_obj['5'][shop_id]:0,
      'week4':(shop_id in u_count_obj['4'])? u_count_obj['4'][shop_id]:0,
      'week3':(shop_id in u_count_obj['3'])? u_count_obj['3'][shop_id]:0,
      'week2':(shop_id in u_count_obj['2'])? u_count_obj['2'][shop_id]:0,
      'week1':(shop_id in u_count_obj['1'])? u_count_obj['1'][shop_id]:0,
      'week0':(shop_id in u_count_obj['0'])? u_count_obj['0'][shop_id]:0,
      'phone': phone,
      'address': address
    });
  }
  usage_season_report = usage_season_report.sort(function (a, b) {
    return a.id - b.id;
  });
  
  // 創建寫入流
  const ws = fs.createWriteStream(`./usage_season_report_${end_date}.csv`);
  // 寫入內容
  ...
  for (let i=0; i<usage_season_report.length; i++) {
    let usr_obj = usage_season_report[i];
