...
async function create_shop_ranking() {
  ...
  */
  ...
  // 讀取 callnum_log 檔案列表
  let filenames = fs.readdirSync("../common_data/callnum_log");
  
  // 統計 叫號機 跳號次數
  let count_arr = {};
  for (let i=0; i<filenames.length; i++) {
    let ymd = path.parse(filenames[i]).name;
    if (ymd >= start_date && ymd <= end_date) {
      let numlog_file = await fs.readFileSync(`../common_data/callnum_log/${filenames[i]}`, 'utf8');
      let numlog_data = await JSON.parse(numlog_file);
      for (let caller_id in numlog_data) {
        if (caller_id in count_arr) count_arr[caller_id] += Object.keys(numlog_data[caller_id]).length;
        else count_arr[caller_id] = Object.keys(numlog_data[caller_id]).length;
      }    
    }
  }
  ...
  for (let shop_id in shop_data) {
    ...
    for (let caller_id in shop_data[shop_id].callers) {
      if (caller_id in count_arr) {
        customer_count += count_arr[caller_id];
      }
    }
