...
export function caller_info(shop_id, caller_id, shop_name, address, caller_name, curr_num, prev_num, change_time, update_time, last_update, booking_url, booking_phone) {
  ...
  */
  ...
  if (booking_url == "" && booking_phone == "") {         // 移除 "立即預約"
    ...
    msg_obj["contents"]["body"]["contents"][6]["contents"][0]["action"] = {"type":"postback", "label":"action", "data":`action=選擇預約方式&shop_name=${shop_name}&booking_url=${encodeURIComponent(booking_url)}&booking_phone=${booking_phone}`};
  }
  ...
  for (let i=0; i<caller_name_arr.length; i++) {
    ...
    if (i==max_num-1 || i==caller_name_arr.length-1) {
      ...
      if (booking_url == "" && booking_phone == "") {         // 移除 "立即預約"
        ...
        item_obj["contents"][1]["action"] = {"type":"postback", "label":"action", "data":`action=選擇預約方式&shop_name=${shop_name}&booking_url=${encodeURIComponent(booking_url)}&booking_phone=${booking_phone}`};
      }
    }
    else {
      item_obj =
...
export function select_booking_type(post_msg) {
  ...
  // 免排預約網頁 需使用外部流灠器
  let booking_external = `${decodeURIComponent(post_msg.booking_url)}&openExternalBrowser=1`;
