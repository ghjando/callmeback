...
async function main() {
  ...
  */
  ...
  // 取得 booking_url (for "取消預約")
  function get_booking_url(shop_id, user_id) {
    let shop_data = shopData;
    
    // 找不到 shop_id 資料
    if (!(shop_id in shop_data)) return "";
  ...
  app.post('/booking', async (req, res) => {
    ...
    let date_key = booking_date.replaceAll("-","");
    let user_name = "";
    let target_name = "";
    let booking_time = "";
    let booking_num = "";
    ...
    // 取得 Week Day
    let date_obj = new Date(booking_date);
    ...
    if (action == "預約") {
      ...
      shop_id = booking_obj.shop_id;

      if (user_id != "") {
        // 有 user_id, 新增至 預約資料
        bookingDB.ref(`${date_key}/${user_id}/${booking_time_id}`).set(booking_obj);
        // 準備通知訊息
        let booking_url = get_booking_url(shop_id, user_id);
        notify_Msgs.push(reply_msg.booking_info(target_name, user_name, booking_date, week_day, booking_time, booking_num, booking_url));
      }
      else {
        // 缺少 user_id, 新增至 預約暫存資料
        bookingTempDB.ref(`${date_key}/${user_name}/${booking_time}`).set(booking_obj);    
      }
    }
    else if (action == "取消預約") {
      // 資料庫 讀取預約資料
      ...
      if (Object.keys(bookingData).length > 0) {
        ...
        booking_num = bookingData.booking_num;
        ...
        let msg = `${booking_date}預約資料不存在或已取消`;
        notify_Msgs.push({type:'text', text:msg});
      }
    }
    ...
    if (Object.keys(bookingData).length > 0) {
      for (let user_id in bookingData) {
        ...
        for (let booking_time_id in bookingData[user_id]) {
          ...
          let booking_date = bookingData[user_id][booking_time_id].booking_date;
          let booking_time = bookingData[user_id][booking_time_id].booking_time;
          let booking_num = bookingData[user_id][booking_time_id].booking_num;
